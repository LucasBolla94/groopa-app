...
export default function ListOwnerPage({
  ...
  params: Promise<{ idlist: string }>
}) {
  const { idlist } = use(params)
  ...
  useEffect(() => {
    ...
    const listRef = doc(db, 'list', idlist)
    const unsubList = onSnapshot(listRef, snap => {
      ...
      const data = snap.data() as Omit<ListDoc, 'id'>
    ...
    const itemsRef = query(
      collection(db, 'list', idlist, 'items'),
  ...
  async function addItem(e: FormEvent<HTMLFormElement>) {
    ...
    if (!name) return
    await addDoc(collection(db, 'list', idlist, 'items'), {
      name,
  ...
  async function togglePicked(item: ItemDoc) {
    await updateDoc(doc(db, 'list', idlist, 'items', item.id), {
      picked: !item.picked,
    })
  }

  async function deleteItem(id: string) {
    await deleteDoc(doc(db, 'list', idlist, 'items', id))
  }
